...
public class emergency extends AppCompatActivity implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, LocationListener {
    ...
    protected void onResume() {
        ...
        if (!checkPlayServices()) {
            Toast.makeText(this, "Você precisa instalar Google Play Services para utilizar essa funcionalidade", Toast.LENGTH_SHORT).show();
        }
    }
...
//            locationTv.setText("Latitude : " + location.getLatitude() + "\nLongitude : " + location.getLongitude());
    ...
    private void enableSmsButton() {
        try {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                String msg = "ESTOU NUMA SITUAÇÃO DE EMERGÊNCIA, PRECISO DE AJUDA!"+ "\nMinha Localização: " + "\nLatitude : " + location.getLatitude() + "\nLongitude : " + location.getLongitude();
                SmsManager.getDefault().sendTextMessage(num, null, msg, null, null);
            Toast.makeText(getApplicationContext(), "SMS enviado com sucesso", Toast.LENGTH_SHORT).show();
        } }catch (Exception e) {
            Toast.makeText(getApplicationContext(), "Falha ao enviar SMS, por favor, tente novamente", Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void startLocationUpdates() {
        ...
        if (ActivityCompat.checkSelfPermission(this,
                ...
                &&  ActivityCompat.checkSelfPermission(this,
                Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
            Toast.makeText(this, "Você precisa ativar a permissão para mostrar localização !", Toast.LENGTH_SHORT).show();
        }
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                        if (shouldShowRequestPermissionRationale(permissionsRejected.get(0))) {
                            new AlertDialog.Builder(emergency.this).
                                    setMessage("Você precisa ativar a permissão para mostrar a localização.").
